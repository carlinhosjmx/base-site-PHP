...
class AdminEmpresa {
     ...
     */
    public function cadastraEmp(array $Data) {
        $this->Data = $Data;
        if (in_array('', $this->Data)):
            $this->Error = ["Erro ao Cadastrar: Para cadastrar uma empresa, preencha todos os campos!", JMX_ALERT];
            $this->Result = false;
        else:
            $this->setData();
            $this->setName();
     ...
     * @param ARRAY $Data = Atribuitivo
     */
    public function updateDb($EmpresaId, array $Data) {
        $this->Empresa = (int) $EmpresaId;
        $this->Data = $Data;
        if (in_array('', $this->Data)):
            $this->Error = ["Erro ao Atualizar: Para atualizar <b>{$this->Data['empresa_title']}</b>, preencha todos os campos!", JMX_ALERT];
            $this->Result = false;
        else:
            $this->setData();
            $this->setName();
     ...
     * @param INT $EmpresaId = Id da empresa!
     */
    public function empDelete($EmpresaId) {
        $this->Empresa = (int) $EmpresaId;

        $ReadEmp = new Read;
        $ReadEmp->readDb(self::Entity, "WHERE empresa_id = :emp", "emp={$this->Empresa}");
        if (!$ReadEmp->getResult()):
            $this->Error = ["A empresa que você tentou deletar não existe no sistema!", JMX_ERROR];
            $this->Result = false;
        else:
            ...
            $this->Error = ["A empresa <b>{$EmpDelete['empresa_title']}</b> foi removida com sucesso do sistema!", JMX_ACCEPT];
            $this->Result = true;
        endif;
    }
    ...
    public function ExeStatus($EmpresaId, $EmpresaStatus) {
        ...
        $Update = new Update;
        $Update->updateDb(self::Entity, $this->Data, "WHERE empresa_id = :id", "id={$this->Empresa}");
    }
    ...
    private function setData() {
        $this->Data['empresa_name'] = Check::urlFormat($this->Data['empresa_title']);
    ...
    private function setName() {
        ...
        $ReadName = new Read;
        $ReadName->readDb(self::Entity, "WHERE {$Where} empresa_title = :t", "t={$this->Data['empresa_title']}");
    ...
    private function sendCapa() {
        if (!empty($this->Data['empresa_capa']['tmp_name'])):
            ...
            if ($w != '578' || $h != '288'):
                $this->Error = ['Capa Inválida: A Capa da empresa deve ter exatamente 578x288px do tipo .JPG, .PNG ou .GIF!', JMX_INFO];
                $this->Result = false;
            else:
                $this->checkCover();
                $Upload = new Upload;
    ...
    private function cadastra() {
        ...
        if ($Insert->getResult()):
            ...
            $this->Error = ["A empresa <b>{$this->Data['empresa_title']}</b> foi cadastrada com sucesso no sistema!", JMX_ACCEPT];
        endif;
    }
    ...
    private function Update() {
        ...
        if ($Update->getRowCount() >= 1):
            $this->Error = ["A Empresa <b>{$this->Data['empresa_title']}</b> foi atualizada com sucesso!", JMX_ACCEPT];
            $this->Result = true;
        endif;
    }
