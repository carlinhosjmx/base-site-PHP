<div class="content list_content">
    ...
    <section class="list_emp">
        ...
        if ($empty):
            ErroJMX("Oppsss: Você tentou editar uma empresa que não existe no sistema!", WS_INFOR);
        endif;
        ...
        if ($action):
            ...
            switch ($action):
                case 'active':
                    ...
                    ErroJMX("O status da empresa foi atualizado para <b>ativo</b>. Empresa publicado!", WS_ACCEPT);
                    break;

                case 'inative':
                    $empUpdate->ExeStatus($empAction, '0');
                    ErroJMX("O status da empresa foi atualizado para <b>inativo</b>. Empresa agora é um rascunho!", WS_ALERT);
                    break;

                case 'delete':
                    $empUpdate->ExeDelete($empAction);
                    ErroJMX($empUpdate->getError()[0], $empUpdate->getError()[1]);
                    break;
        ...
        $readEmp->readDb("app_empresas", "ORDER BY empresa_status ASC, empresa_title ASC LIMIT :limit OFFSET :offset", "limit={$Pager->getLimit()}&offset={$Pager->getOffset()}");
        if ($readEmp->getResult()):
            foreach ($readEmp->getResult() as $emp):
                ...
                $stateObj->readDb("app_estados", "WHERE estado_id = :est", "est={$empresa_uf}");
                $state = ($stateObj->getResult() ? $stateObj->getResult()[0]['estado_uf'] : 'NULL');

                $cityObj = clone($readEmp);
                $cityObj->readDb("app_cidades", "WHERE cidade_id = :city", "city={$empresa_cidade}");
            ...
            $Pager->ReturnPage();
            ErroJMX("Desculpe, ainda não existem empresas cadastradas!", JMX_INFO);
        endif;
        ?>
