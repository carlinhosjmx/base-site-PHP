<div class="content form_create">
    ...
    <article>
        ...
        if ($data && $data['SendPostForm']):
            ...
            $cadastra->updateDb($empresa, $data);

            ErroJMX($cadastra->getError()[0], $cadastra->getError()[1]);
        else:
            $readEmp = new Read;
            $readEmp->readDb("app_empresas", "WHERE empresa_id = :emp", "emp={$empresa}");
        ...
        if ($checkCreate && empty($cadastra)):
            ErroJMX("A empresa <b>{$data['empresa_title']}</b> foi cadastrada com sucesso no sistema!", JMX_ACCEPT);
        endif;
        ?>
        ...
        <form name="PostForm" action="" method="post" enctype="multipart/form-data">
            ...
            <div class="label_line">
                <label class="label_medium">
                    ...
                    <input type="url" placeholder="http://www.jmxrio.com.br" name="empresa_site" value="<?php if (isset($data['empresa_site'])) echo $data['empresa_site']; ?>" />
                </label>
            ...
            <div class="label_line">
                <label class="label_small">
                    ...
                    <select class="j_loadstate" name="empresa_uf">
                        ...
                        <?php
                        $readState = new Read;
                        $readState->readDb("app_estados", "ORDER BY estado_nome ASC");
                ...
                <label class="label_small">
                    ...
                    <select class="j_loadcity" name="empresa_cidade">
                        <?php if (!isset($data['empresa_cidade'])): ?>
                            ...
                            $City = new Read;
                            $City->readDb("app_cidades", "WHERE estado_id = :uf ORDER BY cidade_nome ASC", "uf={$data['empresa_uf']}");
                            if ($City->getRowCount()):
