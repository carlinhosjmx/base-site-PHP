<div class="content form_create">
    ...
    <article>
        ...
        if ($ClienteData && $ClienteData['SendPostForm']):
            ...
            $cadastra->updateDb($userId, $ClienteData);

            ErroJMX($cadastra->getError()[0], $cadastra->getError()[1]);
        else:
            $ReadUser = new Read;
            $ReadUser->readDb("ws_users", "WHERE user_id = :userid", "userid={$userId}");
        ...
        if ($checkCreate && empty($cadastra)):
            ErroJMX("O usuário <b>{$ClienteData['user_name']}</b> foi cadastrado com sucesso no sistema!", JMX_ACCEPT);
        endif;
        ?>
