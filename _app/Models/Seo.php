...
class Seo {
    ...
    private function getSeo() {
        ...
        switch ($this->File):
            //SEO:: POST
            case 'index':
                $Admin = (isset($_SESSION['userlogin']['activo']) && $_SESSION['userlogin']['activo'] == 3 ? true : false);
                

                $Check = ($Admin ? '' : 'activo = 1 AND');

                $ReadSeo->readDb("cadastro", "WHERE {$Check} email = :link", "link={$this->Link}");
