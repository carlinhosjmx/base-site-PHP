...
class AdminSistema{
    ...
    public function cadastrar(array $Data) {
        ...
        $readBanco = new Read;
        $readBanco->readDb(self::Entity, "WHERE cpf = :cpf", "cpf={$this->Data['cpf']}");
        if( $readBanco->getResult() ):

            echo'<script language="JavaScript">  swal({ title:"Ops, Cliente já cadastrado!",  confirmButtonColor: "#1a3862" }); </script>';
    ...
    private function setData() {
         ...
         //  $separa = explode("/",$this->Data['data']);
         
        $this->Data = array_map('strip_tags', $this->Data);
        $this->Data = array_map('trim', $this->Data);

      
        $this->Data['data_cad'] =   Check::formatData($this->Data['data']);

            

        $this->Data['data_consulta'] = Check::formatData(date('d/m/Y', strtotime(' + 20 days')));
	
        $this->Data['cpf'] = $this->Data['cpf'];
        $this->Data['telefone_1'] = $this->Data['tel_1'];
        $this->Data['registro_cnh'] = $this->Data['registro_cnh'];

    

        

        unset($this->Data['tel_1'],$this->Data['end'],$this->Data['data'], $this->Data['post_status']);
   ...
   /* private function setName() {
        ...
        $readName = new Read;
        $readName->readDb(self::Entity, "WHERE {$Where} nome_cli = :t", "t={$this->Data['nome_cli']}");
    ...
    private function exeCadastra() {
        ...
        if ($cadastra->getResult()):
            ...
            $this->Result = $cadastra->getResult();
              ...
              header("Location: painel.php?exe=sistema/docs&cod={$this->Data['cpf']}");

        else: 
        
        echo 'Não cadastrou no banco!';
